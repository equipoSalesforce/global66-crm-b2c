...
function WidgetHeatmap({ data }: { data: Array<{ label: string; value: number }> }) {
  const max = Math.max(...data.map((item) => item.value), 1);
  return <div className="grid h-full grid-cols-3 gap-2">{data.slice(0, 9).map((item) => <div key={item.label} className="flex min-h-16 flex-col items-center justify-center rounded-lg text-center text-white shadow-sm" style={{ backgroundColor: "var(--g66-brand-blue)", opacity: Math.max(0.28, item.value / max) }}><strong className="text-sm">{formatNumber(item.value)}</strong><span className="mt-0.5 max-w-full truncate px-2 text-[8px]">{item.label}</span></div>)}</div>;
}
