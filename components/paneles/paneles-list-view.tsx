...
export function PanelesListView() {
  ...
  return (
    <div className="space-y-3 pb-6">
      <section className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="rounded-xl bg-blue-50 p-2.5 text-blue-600"><LayoutDashboard className="h-5 w-5" /></span><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-blue-600">Analytics Studio</p><h1 className="text-2xl font-extrabold text-slate-950">Paneles</h1><p className="mt-0.5 text-xs text-slate-500">Crea, organiza y comparte paneles de operación.</p></div></div><div className="flex gap-2"><button type="button" onClick={() => setFolderModal(true)} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-bold text-slate-600 hover:bg-slate-50"><FolderPlus className="h-4 w-4" /> Nueva carpeta</button><Link href="/paneles/nuevo" className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-blue-600 px-3 text-xs font-bold text-white hover:bg-blue-700"><Plus className="h-4 w-4" /> Nuevo panel</Link></div></section>
