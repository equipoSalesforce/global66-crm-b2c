...
export function AiDashboardBuilder() {
  ...
  async function generate() {
    ...
    try {
      ...
      if (!response.ok || !payload.ok || !payload.definition) {
        throw new Error(payload.error || "No se pudo generar el panel.");
      }
      setDefinition(payload.definition);
      setData(payload.data ?? []);
      setMode(payload.mode ?? null);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "No se pudo generar el panel.");
    } finally {
  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok || !payload.ok || !payload.dashboard) throw new Error(payload.error || "No se pudo guardar el panel.");
      router.push(`/dashboards/${payload.dashboard.id}`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "No se pudo guardar el panel.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-4 pb-8">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-start gap-3"><span className="rounded-xl bg-violet-50 p-2.5 text-violet-600"><Bot className="h-5 w-5" /></span><div><p className="text-[9px] font-black uppercase tracking-[0.16em] text-violet-600">Analytics Studio</p><h1 className="text-2xl font-black text-slate-950">Crear panel con IA</h1><p className="mt-1 text-sm text-slate-500">Describe el panel que necesitas y la IA generará una propuesta editable.</p></div></div>
        <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Ej: Necesito un panel con casos cerrados este mes, casos por ejecutivo, casos por cliente y casos en riesgo." className="mt-5 min-h-32 w-full rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm font-semibold leading-6 outline-none transition focus:border-[var(--g66-secondary-interactive)] focus:bg-white focus:ring-2 focus:ring-blue-100" />
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" disabled={isGenerating || prompt.trim().length < 10} onClick={() => void generate()} className="inline-flex h-10 items-center gap-2 rounded-lg bg-blue-600 px-5 text-xs font-black text-white shadow-sm hover:bg-[var(--g66-secondary-interactive)] disabled:opacity-50"><Sparkles className="h-4 w-4" />{isGenerating ? "Generando propuesta..." : "Generar panel"}</button><p className="text-[11px] text-slate-400">La IA sólo puede elegir métricas, dimensiones y widgets aprobados. Nunca ejecuta SQL.</p></div>
      </section>
